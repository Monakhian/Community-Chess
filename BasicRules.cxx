...
RuleSet testRuleSet(std::vector<Rule*>{
    ...
    new MoveGenerator([](ChessBoard& board, MoveGrid& moveGrid, int file, int rank) { // Pawn capture
        if (board.has_piece(file, rank) && board.at(file, rank).get_type() == PAWN) {
        ...
        for (int file = 0; file < 8; file++) {
            for (int rank = 0; rank < 8; rank++) {
                ...
                for (auto& move : moves) {
                    ...
                    // FIXME: Add logic to detect if the player's own king is in check on tempBoard, and if so, remove the move from moves
                }
            }
        }
    })
